...
| `CodeBarreScannerComponent` | Scanner de l'écran de dépôt : **CODE 128 / 39 / 93** (formats qui conservent la chaîne littérale, zéros de tête inclus) **+ QR code** (RG1 : les CB peuvent inclure le QR IDemat). **ITF et UPC/EAN volontairement exclus** (longueur par paires/fixe → ajoutaient/retiraient un `0`, ex. `000003576`). DataMatrix/PDF417 exclus. Fenêtre de visée **carrée** (lit QR et CB dans tous les sens). **Anti-lecture-partielle** : une valeur n'est acceptée qu'après **5 frames identiques consécutives** (`StableBarcodeConfirmer`) + chauffe autofocus. Même API que `BarcodeScannerComponent`. Utilisé dans `AutresCartesScreen`. |
